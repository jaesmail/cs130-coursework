...
const getTracks = async (term) => {
    const res = await fetch(`https://www.apitutor.org/spotify/simple/v1/search?type=track&q=${term}`)
    ...
    outputContainer.innerHTML = tracks.map(track => `
...
const getArtist = async (term) => {
    const res = await fetch(`https://www.apitutor.org/spotify/simple/v1/search?type=artist&q=${term}`)
    const data = await res.json();
